...
# Gemini clients. Gemini Developer API has a free tier suitable for development.
_google_api_key = _require_setting(settings.GOOGLE_API_KEY, "GOOGLE_API_KEY")

_gemini_qna_llm = ChatGoogleGenerativeAI(
...
async def question_answering_agent(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        response = await _gemini_qna_llm.ainvoke([HumanMessage(content=prompt)])
