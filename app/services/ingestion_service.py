...
class IngestionService:
    ...
    def __init__(self):
        """Initialize ingestion service with Gemini and Pinecone clients."""
        google_api_key = settings.GOOGLE_API_KEY
        if not google_api_key:
            raise ValueError("GOOGLE_API_KEY configuration is required")
        self.metadata_llm = ChatGoogleGenerativeAI(
...
Return the extracted metadata as a JSON object matching the schema exactly. All values must be strings."""
    ...
    async def extract_metadata(
        ...
        try:
            logger.info("Extracting metadata using Gemini")
            response = await self.metadata_llm.ainvoke([HumanMessage(content=prompt)])

            # Parse JSON response
            metadata_json = str(response.content).strip()
            if metadata_json.startswith("```"):
                metadata_json = metadata_json.split("\n", 1)[-1].rsplit("```", 1)[0].strip()
    ...
    def generate_embedding(self, text: str) -> List[float]:
        """
        Generate an embedding with Gemini Embedding.
        
        Args:
            text: Text to embed
            
        Returns:
            Embedding vector with the configured dimension.
        """
        try:
